...
var MTMCodeFrame = "code", MTMenuFrame = "menu", MTMTableWidth = "100%", MTMenuImageDirectory = "/menuimages/menu-tree-images/";
...
var MTMenuText = '���ܲ˵�', MTMRootIcon = "menu_new_root.gif", MTMRootColor = "#000000";
...
function MTMDisplayMenu() {
	if(MTMUA.MTMable && !MTMUpdating) {
		...
		if(MTMUA.DOMable) {
			...
			MTMOutputString += '<link rel="stylesheet" href="/css/thickbox.css" type="text/css" media="screen" />\n';		
			if(MTMLinkedSS) {
		...
		if(!MTMUA.DOMable && (MTMFirstRun || MTMUA.browserType != "IE")) {
			...
			MTMUA.document.writeln('\n<a href="/lockedBlank.html?height=-1&width=-1&modal=true" class="thickbox" id="mtmScreenLock" title="Screen Lock">login (modal)</a>\n</div>');
			MTMUA.document.writeln('\n</body>');
			MTMUA.document.writeln('\n<script type="text/javascript">');
			MTMUA.document.writeln('\nfunction locked(){ \n');
			MTMUA.document.writeln('\nvar obj = document.getElementById("mtmScreenLock");');			
			MTMUA.document.writeln('\nvar ie = (window.navigator.appName == "Microsoft Internet Explorer") ? true : false;');	
			MTMUA.document.writeln('\nif (!ie) {');	
			MTMUA.document.writeln('\nvar evt = document.createEvent(\'MouseEvents\');');	
			MTMUA.document.writeln('\nevt.initMouseEvent("click", true, true, window,0, 0, 0, 0, 0, false, false, false, false, 0, null);');	
			MTMUA.document.writeln('\nvar t = obj.dispatchEvent(evt);');	
			MTMUA.document.writeln('\nif(t)\nobj.click();\n}else\nobj.click();\n}');
			MTMUA.document.writeln('\n');	
